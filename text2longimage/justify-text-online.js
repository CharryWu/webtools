// Unused: copied from https://onlinetexttools.com/justify-text
window.bridges["justify-text"] = function () {
  var o = function (i) {
    var n = this
      , e = u(n);
    if (!e || i.trim().length == 0)
      return "";
    e.useLongestLine && (e.width = l(i));
    for (var t = i.split(/(\n\n+)/), s = "", a = 0; a < t.length; a++) {
      var r = t[a];
      /^\s+$/.test(r) ? s += r : s += justify_text(r, e)
    }
    return s
  };
  function l(i) {
    for (var n = i.split(/\n+/), e = 0, t = 0; t < n.length; t++)
      n[t].length > e && (e = n[t].length);
    return e
  }
  function u(i) {
    var n = i.options.get()
      , e = function (f, g) {
        i.output.showNegativeBadge(f, g, -1)
      }
      , t = n["use-longest-line"]
      , s = n["break-words"]
      , a = n["justify-last-line"];
    if (t)
      var r = null;
    else {
      var r = n["line-length"].trim();
      if (!/^[+-]?\d+$/.test(r))
        return e("Invalid Line Length", "Line length isn't a valid integer."),
          !1;
      if (r = parseInt(r),
        r < 1)
        return e("Invalid Line Length", "Line length must be greater than zero."),
          !1
    }
    return {
      useLongestLine: t,
  ...
  ;
; function word_wrap(l, t) {
  if (t = t || {},
    l == null)
    return l;
  for (var i = t.width || 50, a = t.trimLines || !1, r = l.split(`
`), n = [], e = 0; e < r.length; e++)
    r[e].trim().length == 0 ? n.push(r[e]) : t.breakWords !== !0 ? n.push(textToWordChunks(r[e], i)) : n.push(textToChunks(r[e], i, a));
  return n.join(`
`)
}
function textToWordChunks(l, t) {
  for (var i = new GraphemeSplitter, a = l.split(/(\s+)/g), r = [], n = [], e = 0; e < a.length; e++) {
    var h = a[e]
      , u = i.splitGraphemes(h);
    if (u.length > t && n.length == 0) {
      r.push(u.join(""));
      continue
    }
    n.length + u.length <= t ? (h.trim().length != 0 || n.length != 0) && (n = n.concat(u)) : (r.push(n.join("").trim()),
      n = [],
      e--)
  }
  return n.length != 0 && r.push(n.join("").trim()),
    r.join(`
`)
}
function textToChunks(l, t, i) {
  for (var a = new GraphemeSplitter, r = a.splitGraphemes(l), n = [], e = 0; e < r.length; e += t) {
    if (i) {
      for (var h = 0; !(r[e + h].trim().length > 0); h++)
        ;
      e += h
    }
    var u = r.slice(e, e + t);
...
function longest_line(t) {
  for (var e = new GraphemeSplitter, r = 0, a = 0; a < t.length; a++) {
    var i = e.splitGraphemes(t[a]).length;
    i > r && (r = i)
  }
  return r
}
function justify_text(t, e) {
  var r = Object.assign({
    random,
    width: 55
  }, e);
  lastRandomIndex = 0,
    t = word_wrap(t, r);
  for (var a = t.split(/\r?\n/), i = longest_line(a), s = Math.min(r.width, i), n = a.length, o = "", l = 0; l < n; l++) {
    var v = a[l].trim()
      , f = l !== n - 1 || r.justifyLastLine === !0;
    l === n - 1 && typeof r.justifyLastLine == "function" && (f = r.justifyLastLine(v, s)),
      f && (v = toMaxLength(v, s, r)),
      o += v,
      l < n - 1 && (o += `
`)
  }
...
function toMaxLength(t, e, r) {
  for (var a = new GraphemeSplitter, i = a.splitGraphemes(t).length, s = e - i, n = t.split(" "), o = n.length; s-- > 0;)
    n[r.random(o - 2)] += " ";
  return n.join(" ").trim()
}
function random(t) {
  var e = [.14975434898516649, .48920102670502064, .5460856891554375, .843156677246687, .5339584048702419, .6557222579529927, .025277961886063904, .7906919328816282, .39547316726978843, .8046447313481258, .40539968121122616, .3240593946801942, .04700841115323562]
    , r = e[lastRandomIndex % e.length];
  return lastRandomIndex > 1e3 ? lastRandomIndex = 0 : lastRandomIndex++,
    Math.floor(r * (t + 1))
}
...
typeof module != "undefined" && module.exports && (module.exports = GraphemeSplitter);
;
